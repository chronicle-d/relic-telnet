...
std::string readError(int rtntErrno) {
  ...
  case RTELNET_TCP_ERROR_NOT_CONNECTED:
    return "operation failed, tcp connection was not established.";
...
template <typename T, typename E> T unwrapOrThrow(rtnt::Result<T, E> &&result) {
  if (result.is_err())
...
public:
  ...
  public:
    ...
    Result<sockaddr_in, int>
    getSocketAddr(const char *address, int port = RTELNET_PORT,
                  int ipVersion = RTELNET_IP_VERSION) const {
    ...
    unsigned int Connect(sockaddr_in &address,
                         int ipVersion = RTELNET_IP_VERSION) {
      int sockfd =
          socket((ipVersion == 4) ? AF_INET : AF_INET6, SOCK_STREAM, 0);
      if (sockfd < 0)
    ...
    unsigned int Send(const std::vector<unsigned char> &message, int sockfd,
      ...
      errno = 0;
      ssize_t bytesSent =
          send(sockfd, message.data(), message.size(), sendFlag);
    ...
    Result<std::vector<unsigned char>, int>
    Read(int socketfd, int bufferSize = RTELNET_BUFFER_SIZE,
      ...
      errno = 0;
      ssize_t bytesRead =
          recv(socketfd, reinterpret_cast<char *>(buffer.data()), buffer.size(),
               recvFlag);
...
unsigned int Negotiate(session &session) {
  ...
  while (true) {
    ...
    if (bufferPeekResult.is_err()) {
      return bufferPeekResult.error();
    }
    ...
    std::vector<unsigned char> response = {static_cast<unsigned char>(IAC),
...
unsigned int Connect(session &session) {
  // Get address
  auto addressResult =
      session._tcp.getSocketAddr(session._address, session._port, session._ipv);
